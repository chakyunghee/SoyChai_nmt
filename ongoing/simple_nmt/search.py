...
class SingleBeamSearchBoard():
    ...
    def __init__(
        self,
        device,
        prev_status_config,     # 이게 중요함. temp minibatch 만들 때 들어가는 4가지중 3가지(last hidden state, last cell state, last h-tilde)가 여기에 들어감
        beam_size=5,
        max_length=255,
    ):
        ...
        self.word_indice = [torch.LongTensor(beam_size).zero_().to(self.device) + data_loader.BOS] # <bos>를 beam size개수만큼 만들어 놓기
        # Beam index for selected word index, at each time-step.
        self.beam_indice = [torch.LongTensor(beam_size).zero_().to(self.device) - 1]    #beam size의 개수만큼 0으로 채운 tensor, time step 지날때마다 list에 추가..
        # Cumulative log-probability for each beam.
        self.cumulative_probs = [torch.FloatTensor([.0] + [-float('inf')] * (beam_size - 1)).to(self.device)] # 누적확률
    ...
    def is_done(self):
        ...
        if self.done_cnt >= self.beam_size:     # <eos>가 beam_size(k)개수만큼 나오면 done
            return 1
        return 0
    ...
    def collect_result(self, y_hat, prev_status):
        ...
        for prev_status_name, prev_status in prev_status.items():
            self.prev_status[prev_status_name] = torch.index_select(
                ...
                index=self.beam_indice[-1]  # beam_indice[-1] = [2,2,1]
            ).contiguous()
                    # self.prev_status[prev_status_name] = torch.cat([prev_status[:,2,:], prev_status[:,2,:], prev_status[:,1,:]], dim=1)
