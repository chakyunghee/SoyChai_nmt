...
class MultiHead(nn.Module):
    ...
    def forward(self, Q, K, V, mask=None):
        ...
        QWs = self.Q_linear(Q).split(self.hidden_size // self.n_splits, dim=-1) # 쪼개진 결과물들이  list로 들어있다고.
...
class Transformer(nn.Module):
    ...
    def forward(self, x, y):
        ...
        with torch.no_grad():
            ...
            mask_enc = mask.unsqueeze(1).expand(*x.size(), mask.size(-1))   # encoder에서 <pad>에 mask
            mask_dec = mask.unsqueeze(1).expand(*y.size(), mask.size(-1))   # decoder에서 encoder로 <pad>에 mask
    ...
    def search(self, x, is_greedy=True, max_length=255):    # is_greedy=True : argmax 확률값 가진 단어 뽑겠다.
                                                            # is greedy=False: 랜덤하게 확률 분포상에서 뽑겠다.        
        batch_size = x[0].size(0)    # 추론이라 x만          # max_l 같은 단어가 운없이 계속 나올 경우 끊어주는 작용

        mask = self._generate_mask(x[0], x[1])              # 빈 timestep에 넣는 pad
        x = x[0]
        ...
        while is_decoding.sum() > 0 and len(indice) < max_length:
            ...
            if prevs[0] is None:        # prev none 이면 training
                prevs[0] = h_t
            else:
    ...
    def batch_beam_search(
        ...
        while sum(done_cnt) < batch_size and length <= max_length:  
            fab_input, fab_z, fab_mask = [], [], []    # 초기화            # 여기부터 546줄까지 fabricated minibatch만들기
