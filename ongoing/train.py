...
#    else:
#        model = Seq2Seq(
#            input_size,                     # source scentence embedding 입력크기
#            config.word_vec_size,           # Word embedding vector size
#            config.hidden_size,             # LSTM's hidden vector size
#            output_size,                    # softmax layer output크기
#            n_layers=config.n_layers,       # number of layers in LSTM
#            dropout_p=config.dropout        # dropout-rate in LSTM (encoder와 decoder내의)
#        )

    return model

            # output_size: 내가 예측해야 하는 vocabulary 크기
def get_crit(output_size, pad_index):       # loss function
    # Default weight for loss equals to 1, but we don't need to get loss for PAD token.
    # Thus, set a weight for PAD to zero.
    loss_weight = torch.ones(output_size)   # target y 의 tensor에 pad가 있을 것이므로 pad_index(pad onehot vector)알고 있을 경우 맞출 경우 penalty
    loss_weight[pad_index] = 0.
    # Instead of using Cross-Entropy loss,
    # we can use Negative Log-Likelihood(NLL) loss with log-probability.
    crit = nn.NLLLoss(
        weight=loss_weight,     # pad에 대해 맞추면 penalty(loss weight를 0)
        reduction='sum'         # 원래 default가 mean, 나중에 loss에 직접 bs로 나눠주려고.
    )
...
def get_optimizer(model, config):
    if config.use_adam:
        if config.use_transformer:
            optimizer = optim.Adam(model.parameters(), lr=config.lr, betas=(.9, .98)) # 논문에 pre layer nomalized 의 수치 가져옴
        else: # case of rnn based seq2seq.
            optimizer = optim.Adam(model.parameters(), lr=config.lr)
    elif config.use_radam:
        optimizer = custom_optim.RAdam(model.parameters(), lr=config.lr)
    else:
        optimizer = optim.SGD(model.parameters(), lr=config.lr) # lr을 초기에 1줄 예정(왜냐면 gradient clipping 할 것이므로)

    return optimizer


def get_scheduler(optimizer, config):       # 코드 다름
    if config.lr_step > 0:
...
def main(config, model_weight=None, opt_weight=None):  # config받아서 main함수 실행
    def print_config(config):
        pp = pprint.PrettyPrinter(indent=4)
        pp.pprint(vars(config))
    print_config(config)
                                                # config.lang[:2], config.lang[-2:] --> --lang enko 할 때 앞에 en이 source, ko가 target 
    loader = DataLoader(                                                               # train.en // train.ko 파일 불러오기
        ...
        dsl=False,                              # Turn-off Dual-supervised Learning mode.
                                ...
                                # loader.src.vocab : source language의 vocabulary
    input_size, output_size = len(loader.src.vocab), len(loader.tgt.vocab)
    model = get_model(input_size, output_size, config)
    crit = get_crit(output_size, data_loader.PAD)  # loss function선언 (다음 단어 예측이므로 classfication)
...
if __name__ == '__main__':      # 밖에서 console을 진행하면 여기서 진입.한다고..?
    ...
    main(config)                # config객체를 main함수에 넣으면
