...
def define_argparser():
    ...
    p.add_argument(
        '--beam_size',  # beam search 실행 안하도록 하려면 beam_size를 1로 하기
        type=int,
        default=5,
...
def read_text(batch_size=128):
    ...
    for line in sys.stdin:
        ...
        if len(lines) >= batch_size:    # batch_size만큼 채워지면
            yield lines                 # 뱉어내고(iterative하게 쌓인 애들 뱉어냄)
            lines = []                  # lines비움
...
def get_model(input_size, output_size, train_config, is_reverse=False):
    ...
    model.eval()  # *** 꼭 해주기! 성능 실제로 떨어짐 # We need to turn-on the evaluation mode, which turns off all drop-outs.

    return model


if __name__ == '__main__':      # shell에서 실행
    sys.stdout = codecs.getwriter("utf-8")(sys.stdout.detach())
    config = define_argparser() # predict하기 위한 configuration
    ...
    src_vocab, tgt_vocab, is_reverse = get_vocabs(train_config, config, saved_data)  # is_reverse: dual learning때문에 필요
    ...
    loader = DataLoader()
    loader.load_vocab(src_vocab, tgt_vocab)         # data_loader.py의 def load_vocab(self, src_vocab, tgt_vocab):
    ...
    with torch.no_grad():       # 메모리와 속도 위해 해주기, 지금부터 translate
        ...
        for lines in read_text(batch_size=config.batch_size):
            ...
            lengths         = [len(line) for line in lines]     # raw text임 (문장별, 빈칸으로 split되어 있음), pack sequence하려면 필요함, 문장 길이별로 sorting할 거임
            original_indice = [i for i in range(len(lines))]

            sorted_tuples = sorted(
                zip(lines, lengths, original_indice),
                key=itemgetter(1),
                reverse=True,   # 길이가 긴게 앞으로(위로) 가도록
            )
            ...
            x = loader.src.numericalize(
                loader.src.pad(sorted_lines),           # sorted_lines: 사림이 읽을 수 있는 raw token들
                device='cuda:%d' % config.gpu_id if config.gpu_id >= 0 else 'cpu'
            )
            # |x| = (batch_size, length)    <-- one-hot vector
            ...
            if config.beam_size == 1:
                ...
                sorted_tuples = sorted(zip(output, original_indice), key=itemgetter(1))  # 아까 긴문장 순으로 했음, 원래 인덱스 순서로 바꿔야함
